...
scale = 30.
scale2 = scale*100
window = np.exp(-(np.arange(1000)-500.)**2/40.**2)
window /= window.sum()
window = window*5

window2 = np.exp(-(np.arange(1000)-500.)**2/40.**2)
window2 /= window2.sum()
window2 = window2*5
...
plt.savefig('figs/simple_axon_comparison_filtered.pdf')
plt.savefig('figs/simple_axon_comparison_filtered.png')
