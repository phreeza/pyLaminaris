...
def run_fig1(pot):
  ...
  gs = gridspec.GridSpec(10, 14,bottom=0.18)
  ...
  for ngroup in pot['nodes']:
    for segment in ngroup[0]:
      ...
      ax1.plot(segment[:,1],segment[:,0],color='k',alpha=0.1)
  ...
  for n in range(10):
    for m in range(3):
        ...
        ymin, ymax = plt.ylim()
        dh = ax1.plot(locs[-1][1],locs[-1][0],'.')[0]
  ...
  ax1.contour(pot['loc'][:,1].reshape((70,-1)),pot['loc'][:,0].reshape((70,-1)),amps.reshape((70,-1))/(scalemax*1000000.),0.7**np.arange(10),cmap=plt.cm.jet,norm=norm)
  ax1.fill([1280,1280,1600,1600],[14000,15500,15500,14000],'white',edgecolor='white')
  ...
  #ax1.fill([600,600,1100,1100],[16300,16350,16350,16300],'k')
  ax1.axis('off')
  #plt.ylim(pot['loc'][-1,0]+100,pot['loc'][480,0]-100)
  ax1.set_aspect('equal', 'datalim')
  ax1.set_xlim(-100,1300)
  ax1.set_ylim(locs[-1,0]+100,locs[0,0]-100)

  ax2 = plt.subplot(gs[:,8:11],sharey=ax1)
  bins = np.arange(0,19000,100.)
  hist,edges = np.histogram(node_locs[:,0],bins=bins)
  plt.bar(bottom=bins[:-1],width=hist/100.,height=bins[1],left=0,orientation='horizontal',color='k',edgecolor='w')
  ax2.tick_params(left="off")
  plt.xticks(np.arange(0,18,4))
  plt.xlabel(u'nodes\n[1/\u03BCm]')
  plt.ylim(locs[-1,0]+100,locs[0,0]-100)

  ax3 = plt.subplot(gs[:,11:],sharey=ax1)
  plt.subplots_adjust(left=0.05,right=0.95)
  plt.bar(bottom=bins[1:-1],width=np.diff(hist)/100.,height=bins[1],left=0,orientation='horizontal',color='k',edgecolor='w')
  ...
  sb1_ax= plt.gcf().add_axes([0.05,.0,0.9/14*5,.1],sharex=ax1)
  sb1_ax.axis('off')
...
def run_fig2(pot):
  ...
  for m in range(len(ind)-1):
    plt.plot(pot['loc'][k:k+20,1],normalize(psds[k:k+20,ind[m]:ind[m+1]].mean(axis=1)),'-',color=colors_ind[m],label=str(round(psd_freqs[10*m],1))+'kHz')
  ...
  slopes = np.array([[np.polyfit(np.log(pot['loc'][n*20+1:n*20+20,1]),np.log(psds[n*20+1:n*20+20,m:m+20].mean(axis=1)),1)[0] for m in range(100)] for n in range(30,60)])
  plt.subplot(axarr[:,12:])
  plt.plot(psd_freqs[1:101],slopes.mean(axis=0))
  plt.fill_between(psd_freqs[1:101],slopes.mean(axis=0)+slopes.std(axis=0),slopes.mean(axis=0)-slopes.std(axis=0),alpha=0.2)
  plt.semilogx()
  plt.xlim(psd_freqs[1],psd_freqs[100])

  ind.append(-1)
  for m in range(len(ind)-1):
    plt.fill([max(0.001,psd_freqs[ind[m]]),psd_freqs[ind[m+1]],psd_freqs[ind[m+1]],max(0.001,psd_freqs[ind[m]])],[-2.5,-2.5,-2.6,-2.6],colors_ind[m],edgecolor=None)
    if ind[m] > 0:
...
if __name__ == '__main__':
  pot = np.load('data/parallel_bundle_potential.npz')
  run_fig1(pot)
  run_fig2(pot)
